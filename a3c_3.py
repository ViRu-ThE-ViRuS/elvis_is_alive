...
class Agent(object):
    def __init__(self, gamma, env_name, max_episodes=500, total_cpu=4):
        ...
        self.train_episode, self.test_episode = mp.Value('i', 0), mp.Value('i', 0)
        self.train_results, self.test_results = mp.Queue(), mp.Queue()
    ...
    def run(self):
        ...
        while True:
            try:
                result = self.train_results.get(True, 5)
